            ...
            if state != None:
                for value in state:
                    if value:
                        ...
                        print('System status changed to high state')
                        log_data(__name__, 'warning', 'One of the system components is to hot, starting automatic fan control')
                        return True
            else:
                log_data(__name__, 'error', 'States registered as none, failure in one of the fetch functions')
                return True
        elif high_status:
            log_data(__name__, 'info', 'System already in high state skipping ipmi')
            return True
        else:
            log_data(__name__, 'error', 'Failure in system high state, high_status was not returned as boolean')
            return True
    except Exception as e:
        log_data(__name__, 'error', f'Failed to set server rack fans into automatic mode. Error:{e}')
        return None
def system_low_state(high_status, fan_speed):
    try:
        if high_status:
            ...
            log_data(__name__, 'info', f'Fan control has been set back into manual mode: manual fan speed: {fan_speed}%')
            return False
        else:
            log_data(__name__, 'info', f'Fan control is already set to manual skipping ipmitool instructions. Default fan speed in manual mode: {fan_speed}%')
            return False
    except Exception as e:
        log_data(__name__, 'error', f'An error happened trying to change speed of the fans. Error:{e}')
        return None
...
def check_system_state(cpu_temp, gpu_temp, cpu_status, gpu_status, combined_status, high_status):
    ...
    for value in states:
        if type(value) != bool:
            log_data(__name__, 'error', f'Failure in obtaining value from temp state switch component, manual setting automatic fan control')
            return None
    else:
        log_data(__name__, 'info', 'Value obtained from state switch module without failure')
        return states