...
if __name__ == '__main__':
    while True:
        time.sleep(2)
        status = sysCon.check_system_state(cpu_temp = callTemp.call_cpu_temp(), gpu_temp =callTemp.call_gpu_temp(gpu_installed), cpu_status = cpu_status, gpu_status = gpu_status, combined_status = combined_status, high_status = high_status)
        if status != None:
            ...
            logging_data(__name__, 'critical', 'Program failure, one or more components failed in retrieving data, please manually check the system for status')
        time.sleep(2)